...
# 功能开发规范 (Feature Development Rules)

## 国际化工作流 (Internationalization Workflow)
- **Code for English (以英文为母版设计 UI)**:
  - **开发原则**: 画 UI、写代码布局时，**必须以英文文案长度为准**。
  - **原因**: 英文通常比中文长 1.5-2 倍。如果 UI 能容纳英文，中文显示通常没问题；反之中文适配的 UI 换成英文极易崩坏。
- **Think in Chinese (以中文为核心设计逻辑)**:
  - **产品逻辑**: 核心算法和业务逻辑（尤其是玄学/心理学部分）以中文为母版，确保准确性。
  - **文案创作**: 先撰写精准的中文文案，以此为基准进行意译。
